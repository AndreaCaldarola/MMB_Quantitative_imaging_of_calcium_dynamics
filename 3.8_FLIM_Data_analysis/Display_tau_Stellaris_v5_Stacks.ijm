...
// Current version modified by Sebastian Palacios Martinez


Dialog.create("Input for Lifetime Display");

Dialog.addChoice("What to process ",newArray("Process single file","Process entire directory","Process current image"),"Process current image");
Dialog.addNumber("Minimal Tau for display (ns)  :", 2.0);
Dialog.addNumber("Maximal Tau for display (ns)  :", 5.0);
Dialog.addCheckbox("Automatic determination of Intensity threshold", false);
Dialog.addNumber("Minimum image intensity for lifetime statistics/display:", 100);
Dialog.addNumber("Maximum image intensity for lifetime statistics/display:", 2500);
Dialog.addCheckbox("Zero lifetime image data outside intensity threshold", true);
Dialog.addNumber("Gamma_for_Intensity            :", 0.7);
Dialog.addNumber("Median filter size to smooth lifetime image:", 3);
